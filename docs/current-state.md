...
- `StorageContainer` model with stable `qrID`, name, optional location/details/photo filename, tags, timestamps, and cascade-owned items.
- `InventoryItem` model with name, quantity, optional notes/photo filename, tags, timestamps, and parent container.
...
- Run the app on a physical iPhone and verify create/edit/delete, photo selection/removal, saved location, and all-items filter flows against the on-device SwiftData store.
